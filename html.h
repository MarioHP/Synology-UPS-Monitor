...
String generateHtml(const std::map<String, String>& upsData, const std::map<String, String>& translations, const String& lang = String(DEFAULT_LANGUAGE)) {
  ...
  // Data do tabulky
  for (const auto& entry : upsData) {
    String label = translations.count(entry.first) ? translations.at(entry.first) : entry.first;
    html += "<tr><td>" + label + "</td><td>" + entry.second + "</td></tr>";
  }
  ...
  html += "<p><a href='/update'>Aktualizace firmwaru (ElegantOTA)</a></p>";
